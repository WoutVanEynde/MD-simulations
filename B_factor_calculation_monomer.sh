...
EOL
    ...
    python3 /home/wout/Scripts/1_molecular_dynamics/B_factor/chain_replacer.py 
    vim -c "%s/CTHRA/CTH A/g | %s/NGLYA/NGL A/g" -c "wq" modified_B_factor.pdb
    python3 /home/wout/Scripts/1_molecular_dynamics/B_factor/insert-bfactor.py -p modified_B_factor.pdb -b input_insertion_py.txt -o B_factor_averaged.pdb -d 0.0
    vim -c "%s/CTH A/CTHRA/g | %s/NGL A/NGLYA/g" -c "wq" B_factor_averaged.pdb
    dirname2=$(basename `pwd`)
    mv B_factor_averaged.pdb B_factor_averaged_$dirname2.pdb
    rm extracted*.txt merged_B_factor.txt B_factor.pdb averaged_B_factor.txt averaged_B_factor2.txt input_insertion_py.txt modified_B_factor.pdb
    cd ..
done
